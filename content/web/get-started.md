...
---
This is a quick guide to get started on Web WorldWind basics. We'll go over displaying a WorldWind globe, setting up a development web server, working with placemarks and 3D shapes, and displaying imagery from a map service. The final result should look like this:
...
We have written a basic Web WorldWind app, but unlike the Java and Android versions of WorldWind, Web WorldWind is meant to be part of a web application, so we need a web server to visualize our work. If you're not familiar with local testing servers, [the Mozilla Development Network is a good place to start](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/set_up_a_local_testing_server).
...
- - Open a command line console in the root folder of your web app (where the **index.html** file and your _images_ folder is located).
...
We will use a `WorldWind.Position` object to define the placemark location in terms of latitude, longitude, and altitude (note that the altitude in this case is defined in exponential notation, _e.g._ 1e2 = 100 meters).
...
Our placemark creation looks like:

{{< highlight javascript >}}
var position = new WorldWind.Position(55, -106, 1e2);
...
Now, in terms of latitude, longitude and altitude, we define the boundaries of the polygon using an array of `WorldWind.Position` objects. We can create many shapes, including polygons with interior boundaries. In this case we'll create a triangle 70 km high from the Earth's surface:

{{< highlight javascript >}}
var boundaries = [];
boundaries.push(new WorldWind.Position(20, -75, 7e5));
boundaries.push(new WorldWind.Position(25, -85, 7e5));
boundaries.push(new WorldWind.Position(20, -95, 7e5));
{{< /highlight >}}
...
We can also display more complex 3D shapes from external sources, like [COLLADA](https://www.khronos.org/collada/wiki/COLLADA) 3D models. Let's add one to our current scene.
...
For this feature we will use the commonly used jQuery library. It's not strictly necessary, but it makes life easier. Let's make it the first library we load (before WorldWind itself and our helloWorldWind.js file) via a `<script>` tag in our HTML file:
